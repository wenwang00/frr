...
DEFPY(locator_sid, locator_sid_cmd,
	  "sid WORD behavior <uN | uA | uDT4 vrf VIEWVRFNAME | uDT6 vrf VIEWVRFNAME | uDT46 vrf VIEWVRFNAME>",
      "Configure static local sid\n"
      "Specify SRv6 locator hex sid\n"
	  "Configure behavior\n"
      "Apply the code to an End.uN SID\n"
	  "Apply the code to an End.uA SID\n"
...
DEFPY(no_locator_sid,
	  no_locator_sid_cmd,
	  "no sid WORD behavior <uN | uA | uDT4 vrf VIEWVRFNAME | uDT6 vrf VIEWVRFNAME | uDT46 vrf VIEWVRFNAME>",
      NO_STR
			...
			for (ALL_LIST_ELEMENTS_RO(srv6->sids, zctxnode, zctx)) {
				if (zctx->sid->locator == locator) {
					...
					if (zctx->ctx.behavior == ZEBRA_SEG6_LOCAL_ACTION_END_UN)
						vty_out(vty, " uN");
					else if (zctx->ctx.behavior == ZEBRA_SEG6_LOCAL_ACTION_END_UA) {
						vty_out(vty, " uA");
